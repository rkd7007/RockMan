...
void CMyMapSprite::Create(LPDIRECT3DDEVICE9 pDevice, char* filename)
{
	CMySprite::Create(pDevice, filename);
	SetRect(&m_rcFrame, 0, 0, 1026, 600);
}

void CMyMapSprite::OnDraw()
{
...
void CMyMapSprite::OnUpdate(float time)
{
	OnDraw();
}